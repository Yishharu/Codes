...
tr.data = signal.gausspulse(tr.times() - Sdifftime, fc = 1.0 / sig_freq[0]) + signal.gausspulse(tr.times() - Sdifftime, fc = 1.0 / sig_freq[1]) +  signal.gausspulse(tr.times() - Sdifftime, fc = 1.0 / sig_freq[2])
...
ax2.set_xlim([-40, 120])
plt.suptitle('Gauss Signal Test with frequency' + str(sig_freq) + 's   Norm = ' + str(if_norm))
plt.show()
