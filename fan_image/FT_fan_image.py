...
# Set model titles
titles = ['a. No ULVZ','b. 2 km ULVZ',  'c. 5 km ULVZ']
# Set center frequencies for the Fan
centerfreq=np.linspace(2,35,200)
colors=['r','c','g','y']
...
for i in range(len(events)):
       ...
       if i ==0:
              ax0=plt.subplot(1,len(events)+1,i+1)
       else:
              plt.subplot(1,len(events)+1,i+1)
       event=events[i]
       ...
       for s in [distance]: 
          #try:
              ...
              w0=np.argmin(np.abs(tr.times()-Sdifftime+30))
              w1 = np.argmin(np.abs(tr.times()-Sdifftime-60))

              # Copy trace
              tr1=tr.copy()
              tr1=tr1.filter('bandpass', freqmin=1/40.,freqmax=1/10., zerophase=True)
              ...
              for ft in range(len(centerfreq)):
                     ...
                     trf=trf.filter('bandpass', freqmin=1/(f*1.2),freqmax=1/(f*0.8), zerophase=True)
                     # Add normalized filtered arrival to the Frequency Fan
                     norm=np.max(np.abs(trf.data[w0:w1]))/4.
                     
                     maxima.append(np.argmin(trf.data[w0:w1]))
                     if ft ==0:
       ...
       plt.xlim([-30,60])
#       for r in range(len(maxima)):
#              print(r,trf.times()[w0+maxima[r]]-Sdifftime, centerfreq[r])
#              plt.scatter(trf.times()[w0+maxima[r]]-Sdifftime,centerfreq[r],s=10,c=colors[i])
       ...
       plt.subplot(1,len(events)+1,len(events)+1)
#       for r in range(len(maxima)):
#              print(r,trf.times()[w0+maxima[r]]-Sdifftime, centerfreq[r])
#              plt.scatter(trf.times()[w0+maxima[r]]-Sdifftime,centerfreq[r],s=20,c=colors[i])
                          
                          
filename = '/home/zl382/Pictures/fan_image/Synthetic/'+'FT_fan.png'
