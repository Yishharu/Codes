#!/usr/bin python3
# Usage:   python3 add_travel_times_extra90.py [event] [Phases]
# Example:  python3 add_travel_times_extra90.py 20100320 S P Sdiff Pdiff ...  (edit dir below)
import obspy
...
for s in range(0,len(seislist),1):
    ...
    # calculate the traveltime at distance 90    
    for ph in range(len(phase)):
        test=['taup_time -mod ak135 -km '+str(90*(6371*np.pi/180.))+' -h '+ str(seis[0].stats['evdp']) +' -ph ' + phase[ph]] 
        ...
        try:
            time= float(t[l[0]+1])
            print(phase[ph]+'90',time)
        except:
            time=None
        seis[0].stats.traveltimes[phase[ph]+'90']=time
