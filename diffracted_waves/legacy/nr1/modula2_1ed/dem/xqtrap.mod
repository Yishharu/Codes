MODULE XQTrap; (* driver for routine QTrap *) 
 
   FROM IntElem  IMPORT QTrap;
...
BEGIN 
   a := 0.0; 
   b := pio2; 
   WriteString('Integral of func computed with QTrap'); 
   WriteLn; 
   WriteLn; 
   ...
   WriteLn; 
   QTrap(func, a, b, s); 
   WriteString('Result from routine QTrap is'); 
   WriteReal(s, 12, 6); 
   WriteLn; 
   ReadLn;
END XQTrap.
