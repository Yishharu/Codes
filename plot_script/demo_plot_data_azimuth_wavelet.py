...
for s in range(0,len(seislist),1):
   ...
   # print(s_name +' '+ str(s)+' / '+str(len(seislist)) + ' of '+event)
    ...
    if seis[0].stats['dist'] < 110 and seis[0].stats['dist'] > 100:
        plt.subplot(1,1,1)
    # elif seis[0].stats['dist'] < 110 and seis[0].stats['dist'] > 100:
    #     plt.subplot(1,2,2)        
    else:
        continue
    ...
    data = matlab.double((seistoplot.data[w0:w1]).tolist())
    [wt, f] = eng.cwt(data,40.0,wname,'VoicesPerOctave', Nv,nargout=2)
    FREQRANGE = matlab.double([fmin, fmax])
    data_filter = eng.icwt(wt, f,wname, FREQRANGE,'SignalMean', 0.0,nargout=1)

    seistoplot.data[w0:w1] = np.array(data_filter)
