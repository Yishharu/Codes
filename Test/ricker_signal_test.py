...
centerfreq = np.linspace(2,25,200, endpoint = False)
...
w1 = np.argmin(np.abs(tr.times() - Sdifftime - 200))

w00 = np.argmin(np.abs(tr.times() - Sdifftime + 100))  
w11 = np.argmin(np.abs(tr.times() - Sdifftime - 100))

tr.data[w0:w1] = 0
tr.data[w00:w11] = signal.ricker(w11-w00, 3/np.pi*6**0.5*sig_freq[0])
tr.data[(w00+150):(w11+150)] = tr.data[(w00+150):(w11+150)] + signal.ricker(w11-w00, 3/np.pi*6**0.5*sig_freq[1])
tr.data[(w00-50):(w11-50)] = tr.data[(w00-50):(w11-50)] + signal.ricker(w11-w00, 3/np.pi*6**0.5*sig_freq[2])
...
ax2.set_xlim([-40, 120])
plt.suptitle('Ricker Test with frequency' + str(sig_freq) + 's   Norm = ' + str(if_norm))
plt.show()
