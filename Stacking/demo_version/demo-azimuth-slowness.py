...
# Data Directory
dir = '/raid3/zl382/Data/Hawaii/20100320/fk_analysis/TA.T29A/'
...
print('Execuating Beamforming \
    ...
    \n freq: %.0f s - %.0f s  baz from %.0f to %.0f(\u00B1%.1f)deg slowness from %.2f to %.2f(\u00B1%.2f)'\
    % ('20100320', 'BHT', StartWindow, EndWindow, 1/fmax, 1/fmin, np.rad2deg(baz.min()), np.rad2deg(baz.max()),baz_resolution, sl.min(), sl.max(), sl_resolution))
...
ax.contourf(baz+np.pi,sl,power)
ax.scatter(baz[c]+np.pi,sl[c],c='r',marker='*')
ENDTIME = time.time()
print('BeamForming Excution Time: %s s!' %(ENDTIME-STARTTIME))
if if_BackAzimuth:
    print('Find Peak at baz %.0f deg slowness at %.2f' %(np.rad2deg(baz[c])+180, sl[c]))

    plt.title('Beamforming Power\
        \n Event %s    Real data: %s  Window: %ds - %ds \
        \n freq: %.0f s - %.0f s  baz at %.1f(\u00B1%.1f)deg slowness at %.2f(\u00B1%.2f)'\
        % ('20100320', 'BHT', StartWindow, EndWindow, 1/fmax, 1/fmin, np.rad2deg(baz[c])+180,baz_resolution, sl[c],sl_resolution))
else:
    print('Find Peak at baz %.0f deg slowness at %.2f' %(np.rad2deg(baz[c]), sl[c]))

    plt.title('Beamforming Power\
        \n Event %s    Real data: %s  Window: %ds - %ds \
        \n freq: %.0f s - %.0f s  baz at %.1f(\u00B1%.1f)deg slowness at %.2f(\u00B1%.2f)'\
        % ('20100320', 'BHT', StartWindow, EndWindow, 1/fmax, 1/fmin, np.rad2deg(baz[c]),baz_resolution, sl[c],sl_resolution))
...
for i, st in enumerate(stTOGETHER):
    ...
    if i == 0:
        Linear_Stack = np.zeros(np.shape(long_trace))
        PS_Stack = np.zeros(np.shape(long_trace),dtype='complex128')
        PWS_Stack = np.zeros(np.shape(long_trace))
    Linear_Stack += long_trace /norm
    PHASE = np.unwrap(np.angle(hilbert(long_trace)))
    PS_Stack += np.exp(1j*PHASE)
...
plt.text(time_max+2, -20, 'Phase Stack', fontsize=6)
plt.text(time_max+2, -21, 'Linear Stack', fontsize=6)
plt.text(time_max+2, -22, 'Phase Weighted Stack', fontsize=6)
