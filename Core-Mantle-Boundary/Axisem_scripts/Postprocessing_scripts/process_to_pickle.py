...
open_path = '/raid3/zl382/Synthetics/AxiSEM/'
...
for rnum, run in enumerate(runs):
    ...
    stalist=[]
    dist=[]
    f=open(open_path + runs[0] + '/Data/receiver_names.dat')
    for line in f.readlines():
        val=line.split()
        stalist.append(val[0])
        dist.append(float(val[2]))

    f=open(open_path + runs[0]+'/simulation.info')
    line=f.readlines()[8]
    print(line)
    val=line.split()
    depth = float(val[0])
    ...
    for i in range(len(stalist)): #range(cat.count()):
            ...
            SHref=trace.Trace()
            tmptimes,SHref.data=np.loadtxt(open_path + run + '/Data_Postprocessing/SEISMOGRAMS/'+stalist[i]+'_disp_post_mij_conv0001_Z.dat',unpack=True)
            ...
            SHref=trace.Trace()
            tmptimes,SHref.data=np.loadtxt(open_path + run + '/Data_Postprocessing/SEISMOGRAMS/'+stalist[i]+'_disp_post_mij_conv0001_N.dat',unpack=True)
