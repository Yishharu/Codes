...
######      
for station_path in seislist:
    sname = os.path.split(station_path)[1]
    station = read(station_path,format='PICKLE')
    slat, slon = station[0].stats.stla, station[0].stats.stlo
    if station[0].stats['dist']<= distmax and station[0].stats['dist']>= distmin \
    and station[0].stats['az']<= azmax and station[0].stats['az']>= azmin:
        # plot station 
...
m.scatter(elon, elat,latlon=True,
    s=365,marker='*',facecolors='y',alpha=1)
...
xpt,ypt = 5.69813e+06, 5.98138e+06
...
plt.text(0.9,0.9,'anomoly position: \n lon:%.1f ; lat:%.1f' %(centerlon,centerlat))
