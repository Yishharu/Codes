...
tr.data = np.cos(2 * np.pi * 1.0 / sig_freq[0] * tr.times()) + np.cos(2 * np.pi * 1.0 / sig_freq[1] * tr.times()) + np.cos(2 * np.pi * 1.0 / sig_freq[2] * tr.times())
...
ax2.set_xlim([-40, 120])
plt.suptitle('Sin Signal Test with frequency' + str(sig_freq) + 's   Norm = ' + str(if_norm))
plt.show()
