MODULE XBessJ; (* Driver for routine BessJ *) 

   FROM Bessel IMPORT BessJ;
...
BEGIN 
   ...
   UNTIL Equal(text, 'Bessel Function Jn'); 
   GetInt(dataFile, nValue); 
   GetEOL(dataFile); 
   WriteString(text); 
   WriteLn; 
   WriteString('   n       x          actual      BessJ(n,x)'); 
   WriteLn; 
